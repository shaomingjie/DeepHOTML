...
# The code is for classical MIMO detection with i.i.d. Gaussian channels.
...
def generate_data_train_varychannel_QPSK( B,M,N,Rr_sqrt, Rt_sqrt, snr_low,snr_high):
    ...
    for i in range(B):
        ...
        y =  Hslide.dot(x) 
        Y[ i  ,: ] = y
        sigma[i] = np.sqrt(2*M*N/ SNR[i]  )
        w = sigma[i] * np.random.randn(2*N)
        Ynoise[i  ,:] =  y+ w
...
def relu_layer(x,input_size,output_size,Layer_num):
    ...
    return y
# result analysis
if not os.path.exists('DeepHOTML_Classical_MIMO_Corr_channel/'):
    os.makedirs('DeepHOTML_Classical_MIMO_Corr_channel/')
directory = './DeepHOTML_Classical_MIMO_Corr_channel/'

#parameters
M = 80 # number of receive antennas
N = 80 # number of transmit antennas
# Note that these are complex-valued dimension.


snrdb_low = 28 # training SNR lower bound
snrdb_high = 34 # training SNR upper bound
...
# parameters for test
snrdb_low_test = 20 # test SNR lower bound
snrdb_high_test = 40 # test SNR upper bound
...
for i in range(N):
    for j in range(N):
        Rr[i, j] = r ** abs(i - j)
Rr_sqrt = scipy.linalg.sqrtm(Rr)

Rt = np.zeros([M, M])
for i in range(M):
    for j in range(M):
        Rt[i, j] = r ** abs(i - j)
Rt_sqrt = scipy.linalg.sqrtm(Rt)


# Architecture of DeepHOTML
LayerTied = 20 # number of layers in Deep HOTML
...
with tf.Session() as sess:
    ...
    for i in range(train_iter+1):
        ...
        if i % epoch == 0:
            testbatch_H, testbatch_Y , testbatch_X, testbatch_HY, testbatch_HTH, testbatch_sigma  = generate_data_train_varychannel_QPSK( test_batch_size, M, N,Rr_sqrt, Rt_sqrt, snr_low, snr_high)
            ...
            if i>= 18000:
                ...
                with tf.variable_scope('',reuse=True):
                    ...
                    b1_tf = tf.get_variable(name="relu1/bias", shape=[   2 * N])


                    spio.savemat(directory +'DeepHOTML_last_'+ str(N)+'by'+ str(M) +'SNR'+str(snrdb_low)+'_'+str(snrdb_high)+ 'Layer'+str(LayerTied) + str(int(i/epoch))+'.mat', {
                        'W1': sess.run(W1_tf),
                        'b1': sess.run(b1_tf),
                ...
                with tf.variable_scope('',reuse=True):
                    ...
                    spio.savemat(directory +'DeepHOTML_'+ str(N)+'by'+ str(M) +'SNR'+str(snrdb_low)+'_'+str(snrdb_high)+ 'Layer'+str(LayerTied) + str(int(i/epoch))+'.mat', {
                        'W1': sess.run(W1_tf),
                        'b1': sess.run(b1_tf),
        ...
        batch_H, batch_Y , batch_X, batch_HY, batch_HTH , batch_sigma= generate_data_train_varychannel_QPSK( train_batch_size, M, N, Rr_sqrt, Rt_sqrt,snr_low, snr_high)
        _,trainlambdav,trainTOTAL_LOSS, trainavg_loss, trainBER = sess.run( [train_step,gamma,TOTAL_LOSS, loss2, BER], feed_dict={H: batch_H, Y: batch_Y, X_true: batch_X, HTH: batch_HTH, HY: batch_HY , sigma:batch_sigma})
    ...
    losssigns = np.zeros((1, num_snr))
    tmp_bers = np.zeros((1, test_iter))
    tmp_losssign = np.zeros((1, test_iter))
    ...
    for j in range(num_snr):
        ...
        for jj in range(test_iter):
            batch_H, batch_Y, batch_X, batch_HY, batch_HTH ,batch_sigma= generate_data_train_varychannel_QPSK( test_batch_size, M, N,Rr_sqrt, Rt_sqrt, snr_list[j], snr_list[j])
            tic = tm.time()
